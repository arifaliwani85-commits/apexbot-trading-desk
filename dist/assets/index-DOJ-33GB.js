...
function calculateEMA(prices, period) {
	...
	for (let i = 0; i < prices.length; i++) if (i < period - 1) ema.push(NaN);
	else if (i === period - 1) ema.push(prevEma);
	else {
...
function calculateBollingerBands(prices, period = 20, multiplier = 2) {
	...
	const upper = [];
	const lower = [];
	for (let i = 0; i < prices.length; i++) if (i < period - 1 || isNaN(middle[i])) {
		upper.push(NaN);
		lower.push(NaN);
	} else {
		const stdDev = calculateStandardDeviation(prices.slice(i - period + 1, i + 1), middle[i]);
...
function calculateMACD(prices, fastPeriod = 12, slowPeriod = 26, signalPeriod = 9) {
	...
	const macdLine = [];
	for (let i = 0; i < prices.length; i++) if (isNaN(fastEma[i]) || isNaN(slowEma[i])) macdLine.push(NaN);
	else macdLine.push(fastEma[i] - slowEma[i]);
	const validMacdStart = macdLine.findIndex((val) => !isNaN(val));
	const validSignalLine = calculateEMA(validSignalLinePricesOnly(macdLine.slice(validMacdStart)), signalPeriod);
	function validSignalLinePricesOnly(arr) {
		return arr.map((v) => isNaN(v) ? 0 : v);
	}
	const signalLine = [];
	const histogram = [];
	for (let i = 0; i < prices.length; i++) if (i < validMacdStart + signalPeriod - 1) {
		signalLine.push(NaN);
		histogram.push(NaN);
	} else {
...
function calculateATR(candles, period = 14) {
	...
	for (let i = 0; i < candles.length; i++) if (i < period - 1) atr.push(NaN);
	else if (i === period - 1) atr.push(prevAtr);
	else {
...
function calculateADX(candles, period = 14) {
	...
	for (let i = 1; i < candles.length; i++) {
		...
		const trVal = Math.max(curr.high - curr.low, Math.abs(curr.high - prev.close), Math.abs(curr.low - prev.close));
		tr.push(trVal);
...
function calculateVWAP(candles) {
	...
	for (let i = 0; i < candles.length; i++) {
		...
		const day = new Date(candle.time).getUTCDate();
...
function computeAllIndicators(candles, stratSettings) {
	...
	return candles.map((candle, i) => ({
		...
		bbLower: bb.lower[i],
		rsi: rsi[i],
		macd: isNaN(macdData.macdLine[i]) ? void 0 : {
...
function evaluateStrategy(candles, settings) {
	...
	switch (settings.strategyType) {
		case "TREND_FOLLOWING": return checkTrendFollowing(current, settings, candles);
		case "MEAN_REVERSION": return checkMeanReversion(candles, settings);
		case "MOMENTUM_BREAKOUT": return checkMomentumBreakout(candles, settings);
		case "HIGH_FREQUENCY_SCALPER": return checkHighFrequencyScalper(candles, settings);
		case "NEWS_SENTIMENT_TRADING": return checkNewsSentimentTrading(candles, settings);
		default: return {
			signal: null,
...
function checkTrendFollowing(current, settings, candles) {
	const { ema20, ema50, ema200, rsi, adx, vwap } = current;
	const threshold = settings.adxThreshold !== void 0 ? settings.adxThreshold : 20;
	if (ema20 === void 0 || ema50 === void 0 || ema200 === void 0 || rsi === void 0 || adx === void 0 || vwap === void 0) return {
		signal: null,
		reason: "Indicators not fully calculated"
	};
	if (adx < threshold) return {
		signal: null,
		reason: `ADX (${adx.toFixed(1)}) is below threshold (${threshold}) - choppy sideways market`
	};
	const len = candles.length;
	const lookback = 20;
	if (len >= 21) {
		...
		let totalVol = 0;
		for (let i = len - 2 - lookback; i < len - 2; i++) totalVol += candles[i].volume || 1;
		const avgVol = totalVol / lookback;
		if (previous.volume < avgVol * .8) return {
			signal: null,
			reason: `Low volume on last closed candle: volume (${previous.volume.toFixed(0)}) < 80% of average (${(avgVol * .8).toFixed(0)})`
		};
	}
	const isBullish = ema20 > ema50 && current.close > ema200;
	const isBearish = ema20 < ema50 && current.close < ema200;
	if (isBullish) {
		if (rsi >= settings.rsiOverbought) return {
			signal: null,
			reason: `Uptrend BUY ignored: RSI (${rsi.toFixed(1)}) is overbought (>= ${settings.rsiOverbought})`
		};
		if (!(current.close <= ema20 * 1.008)) return {
			signal: null,
			reason: `Uptrend BUY ignored: Price is too extended above EMA20 ($${current.close.toFixed(2)} vs EMA20 $${ema20.toFixed(2)})`
		};
		if (current.close > vwap * 1.015) return {
			signal: null,
			reason: `Uptrend BUY ignored: Price is too extended above VWAP ($${current.close.toFixed(2)} vs VWAP $${vwap.toFixed(2)})`
		};
		return {
			signal: "BUY",
	...
	if (isBearish) {
		if (rsi <= settings.rsiOversold) return {
			signal: null,
			reason: `Downtrend SELL ignored: RSI (${rsi.toFixed(1)}) is oversold (<= ${settings.rsiOversold})`
		};
		if (!(current.close >= ema20 * .992)) return {
			signal: null,
			reason: `Downtrend SELL ignored: Price is too extended below EMA20 ($${current.close.toFixed(2)} vs EMA20 $${ema20.toFixed(2)})`
		};
		if (current.close < vwap * .985) return {
			signal: null,
			reason: `Downtrend SELL ignored: Price is too extended below VWAP ($${current.close.toFixed(2)} vs VWAP $${vwap.toFixed(2)})`
		};
		return {
			signal: "SELL",
	...
	return {
		signal: null,
		reason: "EMAs or Price not aligned in a trend"
	};
}
function checkMeanReversion(candles, settings) {
	const len = candles.length;
	const current = candles[len - 1];
	const previous = candles[len - 2];
	...
	if (bbLowerCurr === void 0 || bbUpperCurr === void 0 || bbLowerPrev === void 0 || bbUpperPrev === void 0 || rsiCurr === void 0 || adxCurr === void 0 || vwapCurr === void 0) return {
		signal: null,
		reason: "Bollinger Bands / RSI / ADX not fully calculated"
	};
	...
	if (isPriceOversold && isRsiOversold) if (isMarketRanging && isBelowVWAP) return {
		signal: "BUY",
	...
	else if (!isMarketRanging) return {
		signal: null,
		reason: `Mean Reversion Buy ignored: Trend is too strong. ADX is ${adxCurr.toFixed(1)} (>= ${threshold})`
	};
	else return {
		signal: null,
		reason: `Mean Reversion Buy ignored: Price is above VWAP fair value ($${vwapCurr.toFixed(2)})`
	};
	...
	if (isPriceOverbought && isRsiOverbought) if (isMarketRanging && isAboveVWAP) return {
		signal: "SELL",
		...
		signal: null,
		reason: `Mean Reversion Sell ignored: Trend is too strong. ADX is ${adxCurr.toFixed(1)} (>= ${threshold})`
	};
	else return {
		signal: null,
		reason: `Mean Reversion Sell ignored: Price is below VWAP fair value ($${vwapCurr.toFixed(2)})`
	};
	return {
		signal: null,
		reason: "Price is ranging within Bollinger Bands"
	};
}
function checkMomentumBreakout(candles, settings) {
	const len = candles.length;
	const lookback = 20;
	if (len <= lookback) return {
		signal: null,
		reason: "Insufficient lookback data"
	};
	const current = candles[len - 1];
	const previous = candles[len - 2];
	const sliceForHighLow = candles.slice(len - 1 - lookback, len - 1);
	...
	const threshold = settings.adxThreshold || 25;
	if (atrCurr === void 0 || atrPrev === void 0 || adxCurr === void 0 || vwapCurr === void 0) return {
		signal: null,
		reason: "ATR / ADX / VWAP not fully calculated"
	};
	...
	if (isHighBreakout) if (isVolumeExpanding && isAtrRising && hasStrongTrend && current.close > vwapCurr) return {
		signal: "BUY",
	...
	else if (!isVolumeExpanding) return {
		signal: null,
		reason: `Breakout ignored: Volume expansion insufficient (${(current.volume / avgVolume).toFixed(1)}x average)`
	};
	else if (!isAtrRising) return {
		signal: null,
		reason: "Breakout ignored: Volatility (ATR) is not expanding"
	};
	else if (!hasStrongTrend) return {
		signal: null,
		reason: `Breakout ignored: Trend is weak. ADX is ${adxCurr.toFixed(1)} (<= ${threshold})`
	};
	else return {
		signal: null,
		reason: `Breakout ignored: Price is below VWAP fair value ($${vwapCurr.toFixed(2)})`
	};
	if (current.close < lowestLow) if (isVolumeExpanding && isAtrRising && hasStrongTrend && current.close < vwapCurr) return {
		signal: "SELL",
		...
		signal: null,
		reason: `Breakout ignored: Price is above VWAP fair value ($${vwapCurr.toFixed(2)})`
	};
	return {
		signal: null,
		reason: "Price is consolidating within range"
	};
}
function checkHighFrequencyScalper(candles, settings) {
	const len = candles.length;
	if (len < 5) return {
		signal: null,
		reason: "Insufficient data"
	...
	if (ema20 === void 0 || ema50 === void 0 || rsi === void 0) return {
		signal: null,
		reason: "Indicators not fully calculated"
	};
	...
	if (isBullish) {
		if (rsi >= settings.rsiOverbought) return {
			signal: null,
			reason: `Scalp BUY ignored: RSI (${rsi.toFixed(1)}) is overbought (>= ${settings.rsiOverbought})`
		};
		if (!(current.close <= ema20 * 1.01)) return {
			signal: null,
			reason: `Scalp BUY ignored: Price is too extended above EMA20 ($${current.close.toFixed(2)} vs EMA20 $${ema20.toFixed(2)})`
		};
		return {
			signal: "BUY",
	...
	if (isBearish) {
		if (rsi <= settings.rsiOversold) return {
			signal: null,
			reason: `Scalp SELL ignored: RSI (${rsi.toFixed(1)}) is oversold (<= ${settings.rsiOversold})`
		};
		if (!(current.close >= ema20 * .99)) return {
			signal: null,
			reason: `Scalp SELL ignored: Price is too extended below EMA20 ($${current.close.toFixed(2)} vs EMA20 $${ema20.toFixed(2)})`
		};
		return {
			signal: "SELL",
	...
	return {
		signal: null,
		reason: "EMA20 and EMA50 are crossing or flat"
	};
}
function checkNewsSentimentTrading(candles, _settings) {
	const len = candles.length;
	if (len < 5) return {
		signal: null,
		reason: "Insufficient data"
	...
	const current = candles[len - 1];
	const previous = candles[len - 2];
	const hasVolumeSpike = current.volume > 15;
	const isUpward = current.close > previous.close;
	if (hasVolumeSpike) if (isUpward) return {
		signal: "BUY",
...
function generateCandles(regime, length = 500, startPrice = 5e4, timeframeMinutes = 15) {
	...
	const waveCycleLength = 120;
	const waveAmplitude = .02;
	for (let i = 0; i < length; i++) {
		const waveDrift = Math.cos(i / waveCycleLength * Math.PI * 2) * (waveAmplitude / waveCycleLength);
		let localTrend = trendBias + waveDrift;
		if (regime === "VOLATILE" && i > 300 && i < 315) localTrend = -.02;
		if (regime === "VOLATILE" && i >= 315 && i < 330) localTrend = .015;
		const percentChange = localTrend + (Math.random() - .5) * noiseLevel * 2;
		const open = currentPrice;
		const close = currentPrice * (1 + percentChange);
		const maxOC = Math.max(open, close);
		const minOC = Math.min(open, close);
		const wickHighMultiplier = 1 + Math.random() * (noiseLevel * .8);
		const wickLowMultiplier = 1 - Math.random() * (noiseLevel * .8);
		const high = maxOC * wickHighMultiplier;
		const low = minOC * wickLowMultiplier;
		const volume = Math.round(1e3 + Math.random() * 5e3 + Math.abs(percentChange) * 1e5);
		candles.push({
			time: currentTime,
		...
		currentPrice = close;
		currentTime += timeframeMinutes * 60 * 1e3;
	}
	return candles;
}
function generateNextTick(lastCandle, timeframeMinutes = 15, isNewCandle, trendBias = 0) {
	const change = trendBias + (Math.random() - .5) * .001 * 2;
	const newPrice = parseFloat((lastCandle.close * (1 + change)).toFixed(2));
	if (isNewCandle) return {
		time: lastCandle.time + timeframeMinutes * 60 * 1e3,
...
function runBacktest(candles, stratSettings, riskSettings, startingBalance) {
	...
	const profitFactor = totalLossAmount === 0 ? totalWinAmount > 0 ? 99.9 : 0 : totalWinAmount / totalLossAmount;
	let sharpeRatio = 0;
	if (totalTrades > 1) {
...
//#endregion
//#region src/components/TradingChart.tsx
var import_jsx_runtime = (/* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = require_react_jsx_runtime_production();
})))();
var TradingChart = ({ candles, activePosition, closedTrades, showIndicators, symbol = "BTC/USDT" }) => {
	const containerRef = (0, import_react.useRef)(null);
	const mainCanvasRef = (0, import_react.useRef)(null);
	const rsiCanvasRef = (0, import_react.useRef)(null);
	const [visibleCandlesCount, setVisibleCandlesCount] = (0, import_react.useState)(60);
	...
	const drawChart = () => {
		...
		const pricePadding = (maxPrice - minPrice) * .08 || 5;
		...
		const getY = (price) => {
			return chartHeight - (price - minPrice) / (maxPrice - minPrice) * chartHeight + topPadding;
		};
		...
		if (showIndicators.bb) {
			...
			visibleCandles.forEach((c, idx) => {
				...
				} else if (c.bbLower && c.bbUpper) mainCtx.lineTo(getX(idx), getY(c.bbLower));
			});
			for (let idx = visibleCandles.length - 1; idx >= firstValidIdx; idx--) {
				const c = visibleCandles[idx];
				if (c.bbUpper) mainCtx.lineTo(getX(idx), getY(c.bbUpper));
			}
			mainCtx.closePath();
			...
			mainCtx.beginPath();
			visibleCandles.forEach((c, idx) => {
				if (c.bbUpper) if (idx === firstValidIdx) mainCtx.moveTo(getX(idx), getY(c.bbUpper));
				else mainCtx.lineTo(getX(idx), getY(c.bbUpper));
			});
			mainCtx.stroke();
			mainCtx.beginPath();
			visibleCandles.forEach((c, idx) => {
				if (c.bbLower) if (idx === firstValidIdx) mainCtx.moveTo(getX(idx), getY(c.bbLower));
		...
		if (showIndicators.ema20) drawEMALine("ema20", "#f0b90b");
		if (showIndicators.ema50) drawEMALine("ema50", "#2d8cf0");
		if (showIndicators.ema200) drawEMALine("ema200", "#9c52f5");
		const drawVWAPLine = () => {
			mainCtx.beginPath();
			let first = true;
			visibleCandles.forEach((c, idx) => {
		...
		drawVWAPLine();
		const candleWidth = chartWidth / visibleCandles.length * .7;
		visibleCandles.forEach((c, idx) => {
			const x = getX(idx);
			...
			const color = c.close >= c.open ? "#02c076" : "#f6465d";
			mainCtx.strokeStyle = color;
			mainCtx.lineWidth = 1.5;
			mainCtx.beginPath();
		...
		if (activePosition) {
			...
			const drawOrderLine = (price, label, color, style) => {
				const y = getY(price);
				mainCtx.strokeStyle = color;
				mainCtx.lineWidth = 1.2;
				if (style === "dashed") mainCtx.setLineDash([4, 4]);
		...
		rsiCtx.lineWidth = 1;
		rsiCtx.beginPath();
		rsiCtx.moveTo(0, getRsiY(70));
		rsiCtx.lineTo(chartWidth, getRsiY(70));
		rsiCtx.moveTo(0, getRsiY(30));
		rsiCtx.lineTo(chartWidth, getRsiY(30));
		rsiCtx.stroke();
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					scrollOffset > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: ["$", latestPrice.toLocaleString(void 0, { minimumFractionDigits: 2 })]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					style: {
						fontSize: "11px",
						color: "var(--text-secondary)"
					},
					children: [symbol || "BTC/USDT", " (15M Chart)"]
				})]
			}),
...
//#endregion
//#region src/components/StrategyConfig.tsx
var StrategyConfig = ({ stratSettings, setStratSettings, riskSettings, setRiskSettings, botActive, setBotActive, showIndicators, setShowIndicators, botMode, setBotMode, exchangeStatus, onConnectExchange, onDisconnectExchange, symbol, setSymbol, hasUnappliedChanges = false }) => {
	...
	import_react.useEffect(() => {
		if (!exchangeStatus.connected) setIsEditing(false);
	...
	const handleStratParamChange = (param, value) => {
		setStratSettings((prev) => ({
			...prev,
			[param]: value
		}));
	};
	const handleRiskParamChange = (param, value) => {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Cpu, { size: 12 }), " SIMULATOR"]
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Server, { size: 12 }), " EXCHANGE LIVE"]
					})]
				})
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
						...
						style: { opacity: botMode === "EXCHANGE_LIVE" && !exchangeStatus.connected ? .5 : 1 },
						children: botActive ? botMode === "EXCHANGE_LIVE" ? "STOP EXCHANGE BOT" : "STOP SIMULATOR" : botMode === "EXCHANGE_LIVE" ? "START EXCHANGE BOT" : "START SIMULATOR"
					}),
					botMode === "EXCHANGE_LIVE" && !exchangeStatus.connected && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						style: {
							fontSize: "10px",
						...
						children: "⚠️ You must connect your API keys below first."
					}),
					hasUnappliedChanges && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						style: {
			...
			botMode === "EXCHANGE_LIVE" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "sidebar-section",
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h3", {
				...
				}), exchangeStatus.connected && !isEditing ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					style: {
						background: "var(--bg-tertiary)",
						border: "1px solid var(--border-color)",
						borderRadius: "var(--radius-md)",
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								...
								children: exchangeStatus.exchangeId
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							style: {
							...
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: exchangeStatus.isTestnet ? "Testnet paper trading" : "Mainnet Real Trading" })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							style: {
								display: "flex",
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								...
								children: [
									"$",
									exchangeStatus.balance.toFixed(2),
									" USDT"
								]
							})]
						}),
						exchangeStatus.dailyDrawdownPercent !== void 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							style: {
								marginTop: "8px",
								paddingTop: "8px",
							...
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
										style: { color: "var(--text-secondary)" },
										children: "Daily Drawdown:"
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
										style: {
											color: exchangeStatus.dailyDrawdownPercent > 0 ? "var(--accent-red)" : "var(--text-secondary)",
											fontWeight: "bold"
										},
										children: [exchangeStatus.dailyDrawdownPercent.toFixed(2), "%"]
									})]
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									style: {
									...
									children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { style: {
										height: "100%",
										background: exchangeStatus.dailyDrawdownPercent >= riskSettings.maxDailyDrawdown * .8 ? "var(--accent-red)" : "var(--accent-gold)",
								...
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										riskSettings.maxDailyDrawdown,
										"%"
									]
								})
							]
						}),
						exchangeStatus.circuitBreakerTriggered && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							style: {
							...
							children: "🚨 DAILY DRAWDOWN CIRCUIT BREAKER TRIGGERED! Bot deactivated."
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							style: {
								display: "flex",
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [exchangeStatus.connected && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								type: "button",
								className: "btn btn-secondary",
								...
								children: connecting ? "Validating..." : exchangeStatus.connected ? "Save & Connect" : "Connect to Exchange"
							})]
						})
					]
				})]
				...
				children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "form-label",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Risk Per Trade:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
									className: "val",
									children: [riskSettings.riskPercent, "%"]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
								type: "range",
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Info, { size: 10 }),
									" Fits size so SL loss equals exactly ",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "form-label",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Risk-to-Reward (RRR):" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
									className: "val",
									children: ["1:", riskSettings.riskRewardRatio]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
								type: "range",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "form-label",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "ATR SL Multiplier:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
									className: "val",
									children: [riskSettings.atrMultiplier, "x"]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
								type: "range",
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Info, { size: 10 }), " Adjusts Stop Loss distance to asset volatility."]
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Leverage:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
								className: "val",
								children: [riskSettings.leverage, "x"]
							})]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
							className: "select-input",
							...
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: "1",
									children: "1x (No Leverage)"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: "2",
									children: "2x Leverage"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: "5",
									children: "5x Leverage"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: "10",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "form-label",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Max Active Pairs:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "val",
								children: riskSettings.maxConcurrentPositions
							})]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
							type: "range",
							min: "1",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "switch-label",
							style: { cursor: "pointer" },
							onClick: () => handleRiskParamChange("partialTakeProfitEnabled", !riskSettings.partialTakeProfitEnabled),
							children: "Partial Scale-out (Target 1)"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "switch",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
								type: "checkbox",
								checked: riskSettings.partialTakeProfitEnabled,
								onChange: (e) => handleRiskParamChange("partialTakeProfitEnabled", e.target.checked)
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "slider" })]
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						...
						children: "Closes 50% of position at 1.5R, moving remaining SL to break-even to guarantee a risk-free trade."
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "switch-group",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "switch-label",
							style: { cursor: "pointer" },
							onClick: () => handleRiskParamChange("trailingStopEnabled", !riskSettings.trailingStopEnabled),
							children: "Enable Trailing Stop Loss"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "switch",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
								type: "checkbox",
					...
					riskSettings.trailingStopEnabled && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "form-group",
						style: {
							paddingLeft: "12px",
							borderLeft: "2px solid var(--border-color)",
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "form-label",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Trailing Trigger Level:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
									className: "val",
									children: [riskSettings.trailingStopTrigger, " RRR"]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
								type: "range",
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [
									"Moves SL to entry + 20% risk offset when profit hits ",
									riskSettings.trailingStopTrigger,
									"x SL distance."
								]
							})
						]
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", {
							...
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									...
									children: "EMA Golden Cross + RSI"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: "MEAN_REVERSION",
									children: "Bollinger Bands + RSI"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: "MOMENTUM_BREAKOUT",
									children: "Momentum Breakout (ATR)"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: "HIGH_FREQUENCY_SCALPER",
									children: "High-Frequency Scalper (EMA Cross + RSI)"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: "NEWS_SENTIMENT_TRADING",
									children: "News & Whale Sentiment HFT Strategy"
								})
							]
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", {
								className: "form-label",
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Trading Pairs (comma separated):" })
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
								type: "text",
								className: "text-input",
							...
							botActive ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								style: {
									fontSize: "10px",
									color: "var(--text-secondary)",
									marginTop: "2px"
								},
								children: "🔒 Stop the bot to change trading pairs."
							}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								style: {
									fontSize: "9px",
									color: "var(--text-secondary)",
								...
								children: "Enter multiple pairs separated by commas to run a multi-asset portfolio."
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "form-label",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "ADX Filter Threshold:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "val",
									children: stratSettings.adxThreshold || 25
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
								type: "range",
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								...
								children: "Filters entries based on trend strength (ADX > limit for trends, ADX < limit for range-bound)."
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "switch-label",
							style: { cursor: "pointer" },
							onClick: () => handleStratParamChange("useMultiTimeframe", !stratSettings.useMultiTimeframe),
							children: "Multi-Timeframe 4H Filter"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "switch",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
								type: "checkbox",
								checked: stratSettings.useMultiTimeframe,
								onChange: (e) => handleStratParamChange("useMultiTimeframe", e.target.checked)
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "slider" })]
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						...
						children: "Align 15m entry signals with 4H macro EMA trend (EMA20 vs EMA50)."
					}),
					stratSettings.strategyType === "TREND_FOLLOWING" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "form-group",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "form-label",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Short EMA Period:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "val",
								children: stratSettings.emaShortPeriod
							})]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
							type: "range",
							min: "5",
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "form-label",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Long EMA Period:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "val",
								children: stratSettings.emaLongPeriod
							})]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
							type: "range",
							min: "30",
					...
					stratSettings.strategyType === "MEAN_REVERSION" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "form-group",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "form-label",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "RSI Oversold Level:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "val",
								children: stratSettings.rsiOversold
							})]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
							type: "range",
							min: "15",
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "form-label",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "RSI Overbought Level:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "val",
								children: stratSettings.rsiOverbought
							})]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
							type: "range",
							min: "60",
					...
					stratSettings.strategyType === "MOMENTUM_BREAKOUT" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "form-group",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "form-label",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "ATR Period:" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "val",
								children: stratSettings.atrPeriod
							})]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
							type: "range",
							min: "10",
					...
					stratSettings.strategyType === "NEWS_SENTIMENT_TRADING" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						style: {
							fontSize: "11px",
							color: "var(--text-secondary)",
						...
						children: [
							"⚡️ ",
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "News & Whale Sentiment Strategy:" }),
							" Evaluates live crypto headlines from Cointelegraph and Bitcoin.com mixed with real-time order-book whale imbalances to capture high-probability sentiment runs."
						]
					})
				]
			}),
...
//#endregion
//#region src/components/OrderBook.tsx
var OrderBook = ({ latestPrice }) => {
	const [asks, setAsks] = (0, import_react.useState)([]);
	const [bids, setBids] = (0, import_react.useState)([]);
	const [spread, setSpread] = (0, import_react.useState)(0);
	const [spreadPercent, setSpreadPercent] = (0, import_react.useState)(0);
	(0, import_react.useEffect)(() => {
		if (latestPrice === 0) return;
		const rowCount = 6;
		const priceStep = parseFloat((latestPrice * 3e-4).toFixed(2)) || .1;
		const currentSpread = parseFloat((latestPrice * (1e-4 + Math.random() * 4e-4)).toFixed(2));
	...
	}, [latestPrice, Math.floor(Math.random() * 4)]);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "ORDER BOOK" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
				...
				] }) }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: asks.map((ask, idx) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", {
							...
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "orderbook-cell-val",
								children: ask.price.toLocaleString(void 0, { minimumFractionDigits: 2 })
							})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "orderbook-cell-val",
							style: { color: "var(--text-primary)" },
							children: ask.size.toFixed(3)
						}) }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("td", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "depth-bar-asks",
								style: { width: `${ask.percentage}%` }
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "orderbook-cell-val",
								style: { color: "var(--text-secondary)" },
								children: ask.total.toFixed(3)
							})]
						})
					]
				}, `ask_${idx}`)) })]
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "spread-display",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
					className: asks.length > 0 && bids.length > 0 ? Math.random() > .5 ? "green-text" : "red-text" : "",
					children: ["$", latestPrice.toLocaleString(void 0, { minimumFractionDigits: 2 })]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
					style: {
						fontSize: "10px",
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("table", {
				className: "orderbook-table",
				style: { flexGrow: 1 },
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: bids.map((bid, idx) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", {
					className: "orderbook-row",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", {
							...
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "orderbook-cell-val",
								children: bid.price.toLocaleString(void 0, { minimumFractionDigits: 2 })
							})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "orderbook-cell-val",
							style: { color: "var(--text-primary)" },
							children: bid.size.toFixed(3)
						}) }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("td", {
							style: {
								paddingRight: "12px",
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "depth-bar-bids",
								style: { width: `${bid.percentage}%` }
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "orderbook-cell-val",
								style: { color: "var(--text-secondary)" },
								children: bid.total.toFixed(3)
							})]
						})
					]
				}, `bid_${idx}`)) })
...
var TradeLogs = ({ activePosition, closedTrades, logs, onClosePosition, latestPrice, allPositions = [], evaluationStates = {}, accountBalance = 1e4 }) => {
	...
	const getLivePnl = () => {
		...
		const pos = activePosition;
		const usd = (pos.type === "LONG" ? latestPrice - pos.entryPrice : pos.entryPrice - latestPrice) * pos.size * pos.leverage;
		return {
			usd,
			percent: usd / (pos.entryPrice * pos.size) * 100
		};
	};
...
//#endregion
//#region src/components/BacktestPanel.tsx
var BacktestPanel = ({ onRunBacktest, results, riskSettings }) => {
	const [selectedRegime, setSelectedRegime] = (0, import_react.useState)("BULL_TREND");
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "bottom-panel-section",
		style: {
			height: "100%",
			overflowY: "auto"
		},
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
				...
				children: "Test strategy rules over 500 candles"
			})]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			style: {
				padding: "16px",
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
								value: "BULL_TREND",
								children: "📈 Strong Bull Trend"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
								value: "BEAR_TREND",
								children: "📉 Strong Bear Trend"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
								value: "RANGING",
								children: "↔️ Sideways / Ranging"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
								value: "VOLATILE",
								children: "⚡ Volatile Whipsaws & Flash Crash"
							})
						]
					})]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Play, { size: 14 }), " Run Historical Backtest"]
				})]
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										children: ["$", results.netProfitUsd.toLocaleString(void 0, { maximumFractionDigits: 0 })]
									})
								]
							})]
						}),
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										...
										children: "Max Peak-to-Valley loss"
									})
								]
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								...
								className: results.profitFactor >= 1.5 ? "green-text" : results.profitFactor >= 1 ? "gold-text" : "red-text",
								style: {
									fontSize: "16px",
									fontWeight: "bold",
									fontFamily: "var(--font-mono)"
								},
								children: [results.profitFactor.toFixed(2), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									style: {
										fontSize: "10px",
										fontWeight: "normal",
										color: "var(--text-secondary)",
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							...
							className: results.sharpeRatio >= 2 ? "green-text" : results.sharpeRatio >= 1 ? "gold-text" : "red-text",
							style: {
								fontSize: "15px",
								fontWeight: "bold",
								fontFamily: "var(--font-mono)"
							},
							children: results.sharpeRatio.toFixed(2)
						})]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						style: {
							fontSize: "11px",
						...
						children: [
							"💡 ",
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "Expectancy Breakdown:" }),
							" Having a positive expectancy means even with a 45% win rate, enforcing a 1:",
							riskSettings.riskRewardRatio,
							" RRR makes this strategy mathematically profitable in the long term, while restricting single-trade losses to ",
							riskSettings.riskPercent,
							"% protects from capital ruin."
						]
					})]
				})]
			}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: "No backtest run yet. Select a market condition above and click \"Run\" to test your parameters."
			})]
		})]
	});
};
//#endregion
//#region src/components/AuthPage.tsx
var AuthPage = ({ onLoginSuccess }) => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (isLogin) onLoginSuccess(data.token, data.username);
			else {
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h2", {
								...
								children: ["APEXBOT ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "logo-glow",
									style: {
										color: "var(--accent-blue)",
										textShadow: "0 0 12px rgba(45, 140, 240, 0.4)"
									},
									children: "TRADING DESK"
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
								...
								children: "Isolated Multi-Session Algorithmic Futures Bot"
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								...
								children: loading ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [isLogin ? "Access Dashboard" : "Create Account", /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowRight, { size: 16 })]
								})
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Shield, { size: 12 }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "AES-256 encrypted storage. Secure isolated API runs." })]
					})
				]
			})
		]
...
//#region src/App.tsx
function App() {
	const [userToken, setUserToken] = (0, import_react.useState)(localStorage.getItem("userToken"));
	const [username, setUsername] = (0, import_react.useState)(localStorage.getItem("username"));
	...
	const [stratSettings, setStratSettings] = (0, import_react.useState)({
	...
	const [riskSettings, setRiskSettings] = (0, import_react.useState)({
		riskPercent: 1,
		riskRewardRatio: 2,
		atrMultiplier: 2,
	...
	const [botActive, setBotActive] = (0, import_react.useState)(false);
	const [showWelcomeModal, setShowWelcomeModal] = (0, import_react.useState)(true);
	const [botMode, setBotMode] = (0, import_react.useState)("MOCK_SIMULATOR");
	const [exchangeStatus, setExchangeStatus] = (0, import_react.useState)({
		connected: false,
		exchangeId: "binance",
		isTestnet: true,
		balance: 0,
	...
	const [backtestResults, setBacktestResults] = (0, import_react.useState)(null);
	const [evaluationStates, setEvaluationStates] = (0, import_react.useState)({});
	const [hasUnappliedChanges, setHasUnappliedChanges] = (0, import_react.useState)(false);
	...
	const tickCountRef = (0, import_react.useRef)(0);
	const simIntervalRef = (0, import_react.useRef)(null);
	const lastExitTimeRef = (0, import_react.useRef)(null);
	const seenPositionIdsRef = (0, import_react.useRef)([]);
	...
	(0, import_react.useEffect)(() => {
		setCandles(computeAllIndicators(generateCandles("RANGING", 300, 5e4), stratSettings));
	...
	(0, import_react.useEffect)(() => {
		...
		const fetchStatus = async () => {
			try {
				const res = await fetch(`/api/status?symbol=${encodeURIComponent(viewedSymbol)}`, { headers: { "Authorization": `Bearer ${userToken}` } });
				if (res.status === 401) {
					handleLogout();
					return;
				}
				if (res.ok) {
					...
					if (data.allPositions) {
						const currentOpenPositions = data.allPositions.filter((pos) => pos.status === "OPEN");
						const newOpenPosition = currentOpenPositions.find((pos) => !seenPositionIdsRef.current.includes(pos.id));
						if (newOpenPosition) {
							const baseSymbol = newOpenPosition.symbol.split(":")[0];
							setViewedSymbol(baseSymbol);
						}
						seenPositionIdsRef.current = currentOpenPositions.map((pos) => pos.id);
					}
					if (data.evaluationStates) setEvaluationStates(data.evaluationStates);
					...
					if (data.activePosition) setActivePosition(data.activePosition);
					else setActivePosition(null);
					if (data.candles && data.candles.length > 0) setCandles(data.candles);
					setAccount((prev) => ({
						...prev,
						balance: data.balance,
		...
		const fetchLogs = async () => {
			try {
				const res = await fetch("/api/logs", { headers: { "Authorization": `Bearer ${userToken}` } });
				if (res.status === 401) {
					handleLogout();
					return;
				}
	...
	const handleToggleBotActive = async (newActiveState) => {
		if (botMode === "EXCHANGE_LIVE") try {
	...
	(0, import_react.useEffect)(() => {
		if (candles.length > 0) setCandles((prev) => computeAllIndicators(prev, stratSettings));
	}, [
	...
	(0, import_react.useEffect)(() => {
		...
		simIntervalRef.current = setInterval(() => {
			setCandles((prevCandles) => {
				...
				const isNewCandle = tickCountRef.current >= 12;
				if (isNewCandle) tickCountRef.current = 0;
				const bias = stratSettings.strategyType === "TREND_FOLLOWING" ? 15e-5 : 0;
				...
				} else newCandles[newCandles.length - 1] = nextCandleRaw;
	...
	const handleManualClose = async (symbolToClose) => {
		if (botMode === "EXCHANGE_LIVE") {
			try {
				...
				if (res.ok) addLog(`Manual market close order sent to exchange backend successfully for ${symbolToClose || viewedSymbol}.`, "success");
			} catch (e) {
	...
	const handleRunBacktest = (regime) => {
		...
		const results = runBacktest(generateCandles(regime, 500, 5e4), stratSettings, riskSettings, account.balance);
		setBacktestResults(results);
		addLog(`✅ Backtest completed. Net Profit: ${`${results.netProfitPercent >= 0 ? "+" : ""}${results.netProfitPercent.toFixed(2)}%`} (${results.wins}W / ${results.losses}L, Win Rate: ${results.winRate.toFixed(1)}%). Sharpe Ratio: ${results.sharpeRatio.toFixed(2)}. Max Drawdown: -${results.maxDrawdownPercent.toFixed(2)}%.`, "success");
	};
	const getWinRate = () => {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Activity, {
						className: "logo-glow",
						size: 20
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["ApexBot ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						style: {
							fontWeight: 300,
							color: "var(--text-secondary)"
						},
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								...
								className: "metric-value value-mono",
								children: ["$", account.balance.toLocaleString(void 0, { minimumFractionDigits: 2 })]
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "metric-item",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								...
								className: `metric-value value-mono ${account.totalProfit >= 0 ? "green-text" : "red-text"}`,
								children: [
									account.totalProfit >= 0 ? "+" : "",
									"$",
									account.totalProfit.toLocaleString(void 0, { minimumFractionDigits: 2 })
								]
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								...
								className: `metric-value value-mono ${getProfitFactor() >= 1.5 ? "green-text" : getProfitFactor() >= 1 ? "gold-text" : "red-text"}`,
								children: getProfitFactor().toFixed(2)
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "status-badge",
							style: { marginLeft: "12px" },
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: `status-dot ${botActive ? "active" : ""}` }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: botActive ? "LIVE PAPER TRADING" : "SIMULATION IDLE" })]
						}),
						userToken && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							style: {
								display: "flex",
								alignItems: "center",
								gap: "12px",
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(StrategyConfig, {
						stratSettings,
						setStratSettings,
						riskSettings,
						setRiskSettings,
						...
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											...
											children: "Portfolio Basket:"
										}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											className: "badge badge-long",
											children: symbol.split(",").map((s) => s.trim().toUpperCase()).join(" | ")
										})]
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										style: {
											display: "flex",
										...
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											...
											children: symbol.split(",").map((s) => s.trim().toUpperCase()).filter(Boolean).map((s) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
												value: s,
												children: s
											}, s))
										})]
								...
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(BacktestPanel, {
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						...
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(OrderBook, { latestPrice: currentPrice })
					})
				]
			}),
			showWelcomeModal && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "overlay",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ShieldAlert, {
								...
								children: "Preserving Capital: The Low-Loss Framework"
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "risk-alert-box",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "risk-alert-desc",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									...
									children: "\"How do top traders keep the chance of losing money extremely low?\""
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { children: [
									"Professional algorithmic trading relies on ",
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "strict mathematics" }),
									" rather than predicting the future. We have pre-configured this bot with the three core rules top traders use:"
								] })]
							})
						}),
							...
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheckBig, {
										...
										style: { color: "var(--text-primary)" },
										children: "1. The 1% Sizing Rule:"
									}), " The bot adjusts position size dynamically based on Stop Loss. If a trade fails, it loses exactly 1% of the account. It takes 100 straight losses to go broke."] })]
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", {
									style: {
										display: "flex",
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheckBig, {
										...
										style: { color: "var(--text-primary)" },
										children: "2. Volatility-Based Stop Losses (ATR):"
									}), " Stop losses are set outside of normal market noise (calculated via Average True Range), avoiding false stops."] })]
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", {
									style: {
										display: "flex",
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheckBig, {
										...
										style: { color: "var(--text-primary)" },
										children: "3. Positive Mathematical Expectancy (RRR):"
									}), " Setting Take Profits at 2x or 3x the Stop Loss distance ensures that a Win Rate of just 40% makes the system highly profitable."] })]
								})
							]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
